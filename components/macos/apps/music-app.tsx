...
export default function MusicApp() {
  ...
  const [customSongs, setCustomSongs] = useState<CustomYouTubeSong[]>([
    ...
    { 
      ...
      title: '"Subha Hone Na De Full Song"| Desi Boyz | Akshay Kumar ,John Abraham | Pritam | Mika Singh, Kumaar',
    ...
    { 
      ...
      title: 'Old vs New Bangla Mashup I Hasan S. Iqbal I Dristy Anam',
  ...
  const currentPlaylist = useMemo(() => {
    return selectedPlaylist 
      ...
      : { title: 'All Songs', description: 'Your complete music library' };
    ...
    <div className="h-full flex flex-col bg-background" onClick={handleUserInteraction}>
      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex flex-col space-y-1">
              <div 
                className={`flex items-center py-1 px-2 rounded text-sm cursor-pointer ${
                  !selectedPlaylist && !showCustomPlaylist ? 'bg-accent text-accent-foreground' : 'hover:bg-accent/30'
                }`}
                onClick={() => {
              ...
              <div 
                className={`flex items-center py-1 px-2 rounded text-sm cursor-pointer ${
                  showCustomPlaylist ? 'bg-accent text-accent-foreground' : 'hover:bg-accent/30'
                }`}
                onClick={() => {
                  setSelectedPlaylist(null);
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {showYoutubePlayer && currentSong && currentSong.youtubeId && (
          ...
          <div className="p-4 flex items-center">
            <div className="flex-shrink-0 mr-4">
              <div className="w-32 h-32 rounded-md bg-muted overflow-hidden">
                ...
                ) : currentPlaylist && 'coverArt' in currentPlaylist && currentPlaylist.coverArt ? (
                  <Image 
            ...
            <div>
              <h1 className="text-2xl font-bold mb-1">
                {showCustomPlaylist ? 'YouTube Music' : currentPlaylist?.title}
              </h1>
              <p className="text-sm text-muted-foreground mb-2">
                {showCustomPlaylist 
                  ? 'Your songs imported from YouTube Music'
          ...
          <div className="flex-1 overflow-y-auto px-4">
            {isLoading ? (
              <div className="flex items-center justify-center h-32">
                ...
                <span className="ml-2 text-muted-foreground">Loading songs from YouTube...</span>
              </div>
            ) : showCustomPlaylist ? (
              customSongs.length > 0 ? (
                      ...
                      >
                        ...
                        <td className="p-2">
                          <div className="flex items-center">
                            ...
                            <span>{song.title || 'Loading...'}</span>
                          </div>
                        </td>
                        <td className="p-2 text-muted-foreground">{song.artist || 'Loading...'}</td>
                        <td className="p-2">
                          <button 
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="p-1 rounded-full hover:bg-accent/30 text-red-500"
                          >
                            Remove
                          </button>
                        </td>
                ...
                <div className="py-16 flex flex-col items-center justify-center">
                  <Youtube className="w-16 h-16 text-red-500/30 mb-4" />
      ...
      <div className="border-t bg-muted/30 backdrop-blur-sm p-3 flex items-center">
        {currentSong ? (
          <>
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              <div className="flex items-center space-x-2 mb-1">
                ...
                {/* Add explicit unmute button if needed */}
                {showYoutubePlayer && currentSong?.youtubeId && (
                  <button
                    className="ml-3 px-2 py-1 text-xs bg-red-500 text-white rounded hover:bg-red-600 transition-colors"
                    onClick={(e) => {
                      e.stopPropagation();
                      // Forcefully unmute and try to play
                      if (playerRef.current) {
                        try {
                          if (typeof playerRef.current.unMute === 'function') {
                            playerRef.current.unMute();
          ...
          <div className="w-full text-center text-muted-foreground text-sm">
            Select a song to start playing. Audio powered by YouTube Music.
          </div>
        )}
      </div>
